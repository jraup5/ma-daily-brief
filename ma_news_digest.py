...
def ai_enrich(items: list[NewsItem]) -> list[NewsItem]:
    ...
    def _build_prompt(batch: list[dict]) -> str:
        return (
            ...
            "- Deals that are dead — rejected, withdrawn, walked away from, called "
            "off, or terminated\n"
            ...
            "language alone. 'closed' when the headline signals either a completed "
            "deal (completes, completed, closes, finalizes, completion, acquisition "
            "complete, or past-tense 'acquired' in a completion sense) OR a signed "
            "definitive agreement (definitive agreement, signs merger agreement, "
            "agrees to acquire, enters agreement to acquire, reaches deal to "
            "acquire, bare 'to acquire' as in 'Bain to acquire SupplyOn', or bare "
            "'to be acquired'). 'pending' for everything else that is still a "
            "live, unsigned deal: board recommends, board backs, wins bid, wins "
            "auction, makes offer, bids for, submits bid, in talks, advanced "
            "talks, nears deal, explores sale, considers options, rival bid, "
            "bidding war, raises offer, regulatory clearance, awaiting approval, "
            "HSR. CRITICAL OVERRIDE: if the word 'proposed' appears anywhere in "
            "the headline, the status is 'pending' regardless of any other "
            "language — e.g. 'KORE to be acquired at $9.25 per share in proposed "
            "deal' is pending, not closed, because 'proposed' wins. Items whose "
...
/* ── mobile layout skeleton (stage 1) ──────────────────────────────────
   Layout fixes only: no card/table restyle, no chart changes. Those are
   later stages. Desktop (>768px) is untouched -- everything here lives
   inside this one media query. */
@media (max-width: 768px) {
  body { overflow-x: hidden; }

  /* 1. stat tiles: stack vertically instead of a 4-across row */
  .stats { grid-template-columns: 1fr; }
  .stat { border-right: none; border-bottom: 1px solid var(--border); }
  .stat:last-child { border-bottom: none; }

  /* 2. tab nav: bigger tap targets; horizontal scroll (already in place
     via .tabbar's overflow-x) stays the mechanism for overflow tabs */
  .tab { padding: 1rem 1.25rem; font-size: .88rem; }
  .tab-ct { font-size: .74rem; }

  /* 3. headers and large display numbers: scale down to fit */
  .nav { padding: .75rem 1.25rem; flex-wrap: wrap; }
  .nav-brand { font-size: .88rem; }
  .nav-right { gap: .85rem; }
  .nav-date, .nav-meta { font-size: .62rem; }
  .s-val { font-size: 1.4rem; }
  .stat { padding: 1rem 1.25rem; }
  .pending-count { font-size: .72rem; }

  /* 4. deal cards: no literal <table> markup exists here (the deal lists
     are a CSS grid of .card elements) -- the equivalent fix for a grid is
     collapsing it to one column, which is what keeps the page from ever
